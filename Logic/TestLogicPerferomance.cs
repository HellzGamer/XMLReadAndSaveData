...
{
    public class TestLogicPerferomance
    {
        static Stopwatch watch = new Stopwatch();
        static DapperHelper dapperLogic = new DapperHelper();
        string FilePath { get; set; }
        string FilePathToMove { get; set; }
        FileSearchHelper fileHelper;

        XmlSerializer serializer;
        BetradarBetData betData;

        Model.Sport sportToAdd;
        Model.Category categoryToAdd;
        Model.Tournament tournamentToAdd;
        Model.Event matchEventToAdd;
        Model.Competitor competitorToAdd;
        Model.BetType betType;
        Model.Markets marketsToAdd;
            ...
            if (files.Length != 0)
            {
                foreach (var file in files)
                {
                    ...
                    if (file != null)
                    {
                        ...
                        {
                            betData = (BetradarBetData)serializer.Deserialize(fileStream);
